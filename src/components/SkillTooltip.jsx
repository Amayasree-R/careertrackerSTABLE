...
export default function SkillTooltip({ skill, targetJob, anchorRef, onClose }) {
    ...
    const getBadgeStyle = (type) => {
        const styles = {
            'Docs': { bg: 'bg-blue-950', text: 'text-blue-400' },
            'Course': { bg: 'bg-purple-950', text: 'text-purple-400' },
            'Video': { bg: 'bg-red-950', text: 'text-red-400' },
            'Tutorial': { bg: 'bg-green-950', text: 'text-green-400' },
            'Practice': { bg: 'bg-orange-950', text: 'text-orange-400' },
            'Tool': { bg: 'bg-teal-950', text: 'text-teal-400' },
            'Search': { bg: 'bg-[#1e1e2e]', text: 'text-slate-400' },
            'Book': { bg: 'bg-amber-950', text: 'text-amber-400' },
            'Playground': { bg: 'bg-pink-950', text: 'text-pink-400' }
        }
        return styles[type] || { bg: 'bg-[#1e1e2e]', text: 'text-slate-400' }
    }
                    ...
                    {loading && (
                        <div className="flex flex-col items-center justify-center py-12 gap-4">
                            <div className="flex gap-1.5">
                                <div className="w-2.5 h-2.5 bg-violet-500 rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></div>
